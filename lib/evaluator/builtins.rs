...
fn bprint_fn(args: Vec<Object>) -> Result<Object, String> {
    match args.iter().next() {
        ...
        _ => Err(String::from("invalid argument for print")),
    }
}

fn blen_fn(args: Vec<Object>) -> Result<Object, String> {
    match args.iter().next() {
        Some(&Object::IP(ref s)) => Ok(Object::Integer(s.prefix_len() as i64)),
        Some(&Object::List(ref list)) => Ok(Object::Integer(list.len() as i64)),
...
fn btail_fn(args: Vec<Object>) -> Result<Object, String> {
    match args.iter().next() {
        Some(&Object::List(ref arr)) => {
            match arr.len() {
                0 => Err(String::from("empty array")),
                _ => {
                    let tail = &arr[1..];
                    Ok(Object::List(tail.to_vec()))
                }
            }
        }
        _ => Err(String::from("invalid argument for tail")),
    }
}
...
fn bcons_fn(args: Vec<Object>) -> Result<Object, String> {
    ...
    match (args.next(), args.next()) {
        (Some(o), Some(&Object::List(ref os))) => {
            let mut vectors = vec![];
            vectors.push(o.clone());
            for object in os {
                vectors.push(object.clone());
            }
            Ok(Object::List(vectors))
        }
